...
const oldJSExecutor = content.match(/const JSExecutor = \(\) => \{[\s\S]*?(?=const ModuleSummary)/);

const newJSExecutor = `const JSExecutor = () => {
  ...
  const runCode = () => {
    ...
    // Create an import map using data URIs for our virtual files
      ...
      <html>
        <body>
          ...
          <script type="module">
            ...
            // Execute main.js logic via dynamic import to catch errors locally if needed,
            // or just inject it directly as module
            try {
              \${files['main.js']}
            } catch (e) {
...
if (oldJSExecutor) {
  content = content.replace(oldJSExecutor[0], newJSExecutor);
  fs.writeFileSync('src/app/academy/[slug]/page.tsx', content);
} else {
  console.log("Could not find JSExecutor");
}
