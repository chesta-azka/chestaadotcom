...
for (const page of pages) {
  ...
  // Remove the bad SEOProvider injection
  const badInjectionRegex = /<SEOProvider\s+title="[^"]+"\s+description="[^"]+"\s+\/>/g;
  code = code.replace(badInjectionRegex, '');
  
  // Now inject it correctly right after the main return
  // We can look for "export default function " and then the first "return (" after that.
  
  const functionStartIdx = code.indexOf('export default function ');
  if (functionStartIdx !== -1) {
    const returnIdx = code.indexOf('return (', functionStartIdx);
    if (returnIdx !== -1) {
