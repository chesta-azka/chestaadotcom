...
export default function ProjectCard({ project, index, onClick, variants }: ProjectCardProps) {
  ...
  return (
    <motion.div
      className={`group relative overflow-hidden rounded-2xl cursor-pointer border border-gray-200 ${project.className || 'min-h-[400px]'}`}
      variants={variants}
      whileHover={{ scale: 0.98 }}
      transition={{ type: 'spring', stiffness: 300, damping: 20 }}
      onClick={onClick}
    >
      <div className={`absolute inset-0 transition-transform duration-1000 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-110 group-hover:-translate-y-2 group-hover:rotate-1 bg-gray-100`} />
      ...
      <div className="absolute inset-x-8 bottom-8 top-auto flex flex-col justify-end text-left z-10 transition-transform duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:-translate-y-6">
        <div>
          ...
          <h3 className="text-2xl md:text-3xl font-display font-medium tracking-tight text-gray-900 m-0 relative z-10">
            {project.title}
          </h3>
        </div>
        ...
        <div className="mt-4 flex flex-col gap-4 opacity-0 h-0 group-hover:opacity-100 group-hover:h-auto transition-all duration-700 delay-100 overflow-hidden translate-y-4 group-hover:translate-y-0">
          <div className="flex flex-wrap gap-2">
            {techStack.map(tech => (
              <span key={tech} className="px-2.5 py-1 bg-gray-200 backdrop-blur-md rounded-md text-[9px] font-mono font-bold text-gray-200 border border-white/20 uppercase tracking-wider">
                {tech}
              </span>
            ))}
          </div>
          
          <div className="bg-gray-200 backdrop-blur-md px-4 py-2 rounded-xl border border-white/20 inline-flex flex-col items-start w-fit">
