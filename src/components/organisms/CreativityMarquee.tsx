...
const NavbarLogoIcon = () => (
  <span className="inline-flex items-center justify-center mx-6 sm:mx-10 md:mx-12 shrink-0 select-none">
    <span className="relative flex items-center justify-center w-14 h-14 md:w-20 md:h-20 rounded-2xl bg-slate-50 border border-white/15 shadow-[0_12px_36px_rgba(0,0,0,0.6)] group-hover:scale-105 transition-transform duration-500">
      <svg className="w-7 h-7 md:w-10 md:h-10 text-[#6b21a8]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
        <path d="m12 3-8 8 8 8 8-8-8-8z" />
        <path d="m12 8-4 4 4 4 4-4-4-4z" />
      </svg>
      {/* Dynamic breathing neon ring glow without green */}
      <span className="absolute inset-0 rounded-2xl bg-[#6b21a8]/10 opacity-25 blur-md" />
      
      {/* Decorative corner accents */}
      <span className="absolute -top-1 -left-1 w-2.5 h-2.5 border-t-2 border-l-2 border-[#6b21a8]/40 rounded-tl-sm" />
      <span className="absolute -bottom-1 -right-1 w-2.5 h-2.5 border-b-2 border-r-2 border-[#6b21a8]/40 rounded-br-sm" />
    </span>
  </span>
);
