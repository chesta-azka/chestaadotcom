...
const InsightSkeleton = () => (
  <div className="relative flex flex-col h-full bg-slate-50 p-6 md:p-8 rounded-xl border border-slate-100 animate-pulse min-h-[220px] text-left">
    ...
    <section id="insights" className="py-16 md:py-24 relative overflow-hidden bg-white border-b border-slate-100">
      <div className="mx-auto max-w-7xl px-6 relative z-10 w-full">
        ...
        {isLoading ? (
          ...
          <div className="p-8 text-center bg-slate-50 border border-slate-100 rounded-2xl text-slate-600 font-sans">
            {error}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {insights.map((insight, i) => (
              <motion.a 
                ...
                rel="noreferrer"
                key={i} 
                className="group cursor-pointer relative flex flex-col h-full bg-white p-6 md:p-8 rounded-xl border border-slate-200/80 hover:border-[#6b21a8]/40 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 shadow-sm"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
