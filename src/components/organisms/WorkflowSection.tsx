...
export default function WorkflowSection() {
    return (
        <section className="py-24 bg-transparent text-gray-900 relative overflow-hidden">
            ...
            <div className="mx-auto max-w-7xl px-6 relative">
                <div className="text-center mb-16">
                    ...
                    <p className="text-gray-600 text-base font-sans max-w-lg mx-auto">Kami memastikan setiap langkah berjalan optimal untuk hasil terbaik.</p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 relative">
                    {steps.map((step, i) => (
                        <motion.div 
                            ...
                            className="p-6 md:p-8 bg-gray-50 backdrop-blur-md border border-gray-100 rounded-[2rem] hover:border-[#4f46e5]/30 hover:bg-gray-50 transition-all duration-300 relative z-10 flex flex-col group"
                        >
                            <div className="flex justify-between items-start mb-6">
                                <div className="w-12 h-12 rounded-xl bg-gray-100 border border-gray-200 flex items-center justify-center text-[#4f46e5] group-hover:scale-110 transition-transform duration-300">
                                    <step.icon size={22} strokeWidth={1.5} />
                                </div>
                                <span className="text-5xl font-display font-bold text-gray-900/5 group-hover:text-gray-900/10 transition-colors duration-300">{step.id}</span>
                            </div>
                            <h3 className="text-xl font-bold font-sans tracking-tight mb-3">{step.title}</h3>
                            <p className="text-gray-600 text-sm font-sans leading-relaxed flex-grow">{step.desc}</p>
                        </motion.div>
                    ))}
                </div>
            </div>
