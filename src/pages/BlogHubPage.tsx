...
import { AdUnit } from '../components/atoms/AdUnit';
...
export default function BlogHubPage() {
  ...
  }, [combinedAllArticles, selectedCategory, searchQuery]);

  const featuredArticle = filteredArticles.find(a => a.featured) || filteredArticles[0];
  
  const displayArticles = useMemo(() => {
    const base = (selectedCategory === 'All' && searchQuery.trim() === '')
      ? filteredArticles.filter(a => a.slug !== featuredArticle?.slug)
      : filteredArticles;
    
    return base.slice(0, currentPage * postsPerPage);
  }, [filteredArticles, featuredArticle, selectedCategory, searchQuery, currentPage]);

  const hasMore = displayArticles.length < filteredArticles.filter(a => a.slug !== featuredArticle?.slug).length;

  const handleLoadMore = () => {
    setCurrentPage(prev => prev + 1);
  };

  const categories = ['All', 'SEO', 'Strategy', 'Design', 'Performance', 'UI/UX', 'Copywriting', 'General'];

  // Submit new article for Groq AI validation vetting
      ...
      <AnimatePresence mode="wait">
        {activeArticle ? (
          ...
          >
            ...
            <div className="-mx-6 my-16 w-full">
               <CreativityMarquee />
            </div>
          ...
          // STANDARD BLOG MAIN GRID WITH AUTHOR MEMBERSHIP CAPABILITIES
          <motion.div
            key="grid-view"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
