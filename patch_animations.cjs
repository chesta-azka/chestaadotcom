...
const motionWrapper = `<motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.6, ease: "easeOut" }}>`;
...
// Extract everything from <h3 id="project-overview" ... down to <ProjectTimeline />
const oldContentRegex = /<h3 id="project-overview" className="text-2xl sm:text-3xl font-display font-bold mb-5 text-slate-900 tracking-tight leading-snug scroll-mt-32">Project Overview<\/h3>[\s\S]*?<ProjectTimeline \/>/s;

code = code.replace(oldContentRegex, newContent.trim());
fs.writeFileSync('src/app/case-studies/[slug]/page.tsx', code);
console.log('Added motion animations to Case Study sections');
